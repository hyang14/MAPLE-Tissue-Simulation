...
class KeyPressedController(Sofa.Core.Controller):     
	...
	def createNewSphere(self):
		...
		# the Ball is too close to the phantom when initialized. It will create undesired effect
		MO = newSphere.addObject('MechanicalObject', showObject=True, position=[0.05, 0.5, 0.05, 0, 0, 0, 1], name=f'Partical-{self.iteration}', template='Rigid3d') 
		Mass = newSphere.addObject('UniformMass', totalMass=0.02)
